...
$qry = $mysql -> query("SELECT ID, Nev, Member, House FROM playerek WHERE Member = 0 ORDER BY Member DESC");
...
if($mysql -> num($qry)) while($u = $mysql -> object($qry)) {
	if($u -> House > -1) echo '<tr><td>' . $u -> Nev . '</td><td><pre>' .($u -> House == -1 ? "NINCS" : "VAN"). '</pre></td></tr>';
}
