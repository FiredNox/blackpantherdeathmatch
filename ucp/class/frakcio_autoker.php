...
<?
	$sql = mysql_query("SELECT ID, Nev, ".$_f["mezo"].", Online, Admin, UtoljaraAktiv FROM playerek WHERE ".$_f["mezo"]." != '0' ORDER BY ".$_f["mezo"]." DESC, Nev ASC");
	$darab = 0;
	$ma = date("Y-m-d");
	...
	while($player = mysql_fetch_array($sql))
	{
		...
		echo'<tr class="noborder '.($monthActivity >= (30 * 3600 * 7) ? 'uberActive' : ($monthActivity >= (30 * 3600 * 5) ? 'hyperActive' : '')).'">
			...
			echo($player["Online"] == 0 && $player["ID"] != $jatekos["AK"]["ID"] && $player[ $_f["mezo"] ] < $_f["leader"] ||
				 $player["Online"] == 0 && $player["ID"] != $jatekos["AK"]["ID"] && $player[ $_f["mezo"] ] >= $_f["leader"] && $jatekos["Admin"] > $player["Admin"] ?
				'<input type="checkbox" name="nevek[]" id="'.$player["ID"].'_id" value="'.$player["ID"].'">' : '');
			}
		echo'</td></tr>';
	}
	mysql_free_result($sql);
