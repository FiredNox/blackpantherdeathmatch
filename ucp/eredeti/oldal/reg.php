...
if(isset($_POST['reg']))
{
	...
	{
		$rs = mysql_query("SELECT * FROM `ucpuserek` ORDER BY `ID` DESC LIMIT 1");
		$ms = mysql_fetch_assoc($rs);
		$idje = $ms['ID'];
		$idje += 1;
		mysql_query("INSERT INTO ucpuserek (ID, Felhasznalonev, Jelszo, Email) VALUES ('".mysql_escape_string($idje)."', '".mysql_escape_string($n)."','".mysql_escape_string($hashpw)."','".mysql_escape_string($em)."')");
